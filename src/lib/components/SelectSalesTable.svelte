<script lang="ts">
	...
	const handleCheckboxChange = (e: Event) => {
		...
		selectedSales = (
			checkbox.checked ? [...selectedSales, sale] : selectedSales.filter((s) => s.id !== saleId)
		) as SaleWithEmployee[];
...
<div class="mb-6">
	<div class="flex justify-between">
		...
		<div class="p-2">
			<Button
				href={'/app/sales/add?employee=' + (employeeId || '') + '&campaign=' + (campaignId || '')}
				size="sm">Create Sales</Button
			>
		</div>
	</div>
	<Table
	...
	>
		<TableHead class="text-sm text-background-800 font-semibold">
			<TableHeadCell>
				<Checkbox on:change={(e) => selectAll(e)}></Checkbox>
			</TableHeadCell>
