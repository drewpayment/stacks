...
import {
	getPendingSaleOverrides,
	saveManualOverrides,
	createOverridesFromSalesForOverridingManagers
} from '$lib/drizzle/postgres/models/overrides.js';
...
import {
	...
	updateSelectedSalesToPaystub
} from '$lib/drizzle/postgres/models/sales.js';
...
export const load: PageServerLoad = async ({ locals, url }) => {
	...
	const availableExpenseReports = async () => {
		const reports = await db.query.expenseReport.findMany({
			...
			where:
				preloadEmployeeId != null
					? (expenseReport, { and, eq, isNull }) =>
							and(
...
export const actions: Actions = {
	'get-sales-by-employee': async ({ locals, request }) => {
		...
		const sales = await getUnallocatedSalesByEmployee(
			profile?.clientId || '',
			`${campaignId}`,
			`${employeeId}`
		);
	...
	'save-paystub': async ({ locals, request }) => {
		...
		const payload = await request.formData();
		const {
			selectedSales: salesRaw,
			employeeId,
			campaignId,
			selectedSaleOverrides: rawOverrides,
			pendingManualOverrides: rawManualOverrides,
			selectedExpenseReports: rawExpReports
		...
		if (!clientId || !employeeId || !campaignId || !selectedSales?.length)
			error(400, 'Bad Request');

		const pendingPaystub = generatePendingPaystub(
			clientId,
		...
		if (overrideTo) {
			...
			const manualOverridesSaved = await saveManualOverrides(
				clientId,
				pendingPaystub.id,
			...
			pendingPaystub.grossPay +=
				selectedOverrides.reduce((c, v) => c + v.overrideAmount, 0) + manualOverridesSaved.total;
		...
		if (selectedExpenseReports && selectedExpenseReports?.length) {
			const expReportsSaved = await saveExpenseReportsToPaystub(
				paystubSaved.clientId,
				paystubId,
	...
	attacheExpenseReport: async ({ locals, request }) => {
		if (!locals.user) return fail(401, { message: 'Unauthorized' });
		if (!['super_admin', 'org_admin'].includes(locals.user.profile.role))
