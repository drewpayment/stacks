...
<div class="container max-w-5xl">
	<div class="flex">
		<div>
			...
			<p class="mb-4">Create a new paystub for an employee.</p>
		</div>
	</div>
	...
	<div class="grid grid-cols-3 pt-4 gap-2">
		<form
			action="?/get-sales-by-employee"
			method="post"
			class="flex gap-2 justify-between col-span-2"
			use:enhance={({ cancel }) => {
				...
				<div class="flex flex-col space-y-6">
					<Label class="space-y-2">
						...
						<Select
							...
							on:change={handleCampaignSelected}
							required
						/>
					</Label>
				</div>
		...
		<Card class="w-full max-w-md">
			<div class="flex flex-col space-y-6">
				<Label class="space-y-2">
					...
					<Helper class="text-sm">Assign this paystub to a payroll cycle.</Helper>
				</Label>
			</div>
		</Card>
	</div>
	...
	<div class="flex justify-end my-6">
		...
		>
			...
			<input
				type="hidden"
				name="selectedSaleOverrides"
				value={JSON.stringify(selectedSaleOverrides)}
			/>
			<input
				type="hidden"
				name="selectedExpenseReports"
				value={JSON.stringify(selectedExpenseReports)}
			/>
	...
	<div class="my-6">
		<SelectSalesTable bind:data={salesTableData} on:saleSelected={handleSaleSelected}
		></SelectSalesTable>
	</div>

	<div class="my-6">
		<SelectSaleOverridesTable
			bind:data={overrideTableData}
			on:overrideSelected={handleOverrideSelected}
		></SelectSaleOverridesTable>
	</div>
	...
	<div class="my-6">
		...
		<Table
			striped={true}
			shadow={true}
			divClass="bg-background-100 dark:bg-background-300 max-h-80 overflow-y-auto"
		>
							...
							<TableBodyCell>
								{#if report.approvalStatus === 'approved'}
									<Checkbox
										...
										value={report?.id}
										on:change={handleExpenseReportSelected}
									></Checkbox>
								{:else}
