<script lang="ts">
	...
	const { paystub: maybePaystub, campaigns: maybeCampaigns } = data;
	const paystub = maybePaystub as PaystubWith;
	const campaigns = maybeCampaigns as SelectCampaign[];
...
<div class="container max-w-5xl">
	...
	<div class="pb-4">
		<Breadcrumb>
			...
			<BreadcrumbItem href="/app/paystubs">My Pay</BreadcrumbItem>
	...
	<div class="w-full p-2">
		...
		<div class="flex py-4 justify-between items-end">
			<div>
				<h5 class="mb-2 text-md md:text-xl font-bold tracking-tight">
					{paystub.employee.firstName}
					{paystub.employee.lastName}
				</h5>
			...
			<div>
				<p class="mb-2 tracking-tight">
					{#if paystub?.payrollCycle}
						<span class="font-bold text-sm md:text-md">Pay Date:</span>
						<span class="text-sm md:text-md">{formatDate(paystub?.payrollCycle?.paymentDate)}</span>
					{:else}
						<span class="font-bold text-sm md:text-md">Pay Date: </span><span
							class="text-gray-500 italic text-sm md:text-md">Not Assigned to Pay Cycle</span
						>
					{/if}
				</p>
				<p class="mb-2 tracking-tight">
					{#if paystub?.payrollCycle}
						<span class="font-bold text-sm md:text-md">Weekending:</span>
						<span class="text-sm md:text-md">{formatDate(paystub?.payrollCycle?.endDate)}</span>
					{:else}
						<span class="font-bold text-sm md:text-md">Weekending: </span><span
							class="text-gray-500 italic text-sm md:text-md">Not Assigned to Pay Cycle</span
						>
					{/if}
				</p>
		...
		<div class="p-4 border-slate-300 border rounded-lg">
			<div class="flex justify-around gap-6 mt-4">
				<div>
					<h5 class="mb-2 text-md md:text-lg font-bold tracking-tight">Gross Pay</h5>
					<p>
						{formatCurrency(paystub.grossPay)}
					</p>
				</div>
				<div>
					<h5 class="mb-2 text-md md:text-lg font-bold tracking-tight">Total Deductions</h5>
					<p>
						{formatCurrency(paystub.taxDeductions + paystub.otherDeductions)}
					</p>
				</div>
				<div>
					<h5 class="mb-2 text-md md:text-lg font-bold tracking-tight">Net Pay</h5>
					<p>
			...
			<div class="flex justify-around gap-6 mb-8">
				<div>
					<h6 class="mb-2 text-sm md:text-md font-bold tracking-tight">Campaign</h6>
					<p class="text-center text-sm md:text-md">
						{getCampaignName(paystub.campaignId)}
					</p>
				</div>
				<div>
					<h6 class="mb-2 text-sm md:text-md font-bold tracking-tight">Sales</h6>
					<p class="text-center text-sm md:text-md">
						{paystub.totalSales}
					</p>
				</div>
				...
				<div>
					<h6 class="mb-2 text-sm md:text-md font-bold tracking-tight">Overrides</h6>
					<p class="text-center text-sm md:text-md">
				...
				<!-- {/if} -->
				<div>
					<h6 class="mb-2 text-sm md:text-md font-bold tracking-tight">Expenses</h6>
					<p class="text-center text-sm md:text-md">
						{formatCurrency(0)}
					</p>
				</div>
