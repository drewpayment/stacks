    ...
    <div class="flex py-4 justify-between items-end">
      ...
      <div>
        <p class="mb-2 tracking-tight">
          {#if paystub?.payrollCycle}
            <span class="font-bold text-sm md:text-md">Pay Date:</span> <span class="text-sm md:text-md">{formatDate(paystub?.payrollCycle?.paymentDate)}</span>
          {:else}
            <span class="font-bold text-sm md:text-md">Pay Date: </span><span class="text-gray-500 italic text-sm md:text-md">Not Assigned to Pay Cycle</span>
          {/if}
        </p>
        <p class="mb-2 tracking-tight">
          {#if paystub?.payrollCycle}
            <span class="font-bold text-sm md:text-md">Weekending:</span> <span class="text-sm md:text-md">{formatDate(paystub?.payrollCycle?.endDate)}</span>
          {:else}
            <span class="font-bold text-sm md:text-md">Weekending: </span><span class="text-gray-500 italic text-sm md:text-md">Not Assigned to Pay Cycle</span>
          {/if}
        </p>
      </div>
    </div>
