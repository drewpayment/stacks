...
<div class="container max-w-5xl mx-auto p-4">
						...
						{#if paystubs?.length > 0}
							...
							>
								...
								<Select
									...
									size="sm"
									name="paystubId"
									bind:value={paystubId}
									on:change={() => setPaystubBtn.click()}
								/>
								<button type="submit" class="hidden" bind:this={setPaystubBtn}
									>Attach to paystub</button
								>
							</form>
						{:else}
							<span class="text-sm italic"
								>No paystubs available - <a
									href={`/app/payroll/add?ee=${report?.employeeId}`}
	...
	<div class="mt-4 text-sm text-gray-600">
		<p>
			Note: This expense report is associated with the paystub for the pay period {startDate?.toLocaleDateString()}
