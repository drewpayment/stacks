...
export const load = async ({ locals, params }) => {
  ...
  const getPaystubs = async () => {
    ...
    const paystubs = (await db.query.paystub.findMany({
      with: {
        payrollCycle: true,
      },
      where: (paystub, { and, eq, inArray }) => and(
        eq(paystub.employeeId, report?.employeeId as string),
        inArray(paystub.payrollCycleId, cycles.map(c => c.id))
      ),
    })).map(p => ({
      name: `${p.payrollCycle?.startDate} - ${p.payrollCycle?.endDate}`,
      value: p.id,
    }));
